...
def main():
    ...
    img_data = new_in_out.read_jpg_file(img_name + '/' + img_name)
    ...
    change(max_freq, img_data)


def plot(original, changed):
    plt.figure()
    plt.subplot(121)
    new_in_out.show_jpg_sub(original, if_color, 'original')
    plt.subplot(122)
    new_in_out.show_jpg_sub(changed, if_color, 'changed')
    plt.show()
...
def count_auto_correlation_and_print(data, xn, xn_cut):
    ...
    for i in range(start, stop, step):
        row = data[i]
        row_fourier = new_analysis.fourier(row)  # Спектр Фурье строки

        diff.append(new_processing.anti_trend_linear(row))
        diff_fourier = new_analysis.fourier(diff[-1])  # Спектр Фурье производной строки

        acf = new_analysis.auto_correlation(diff[-1])
        acf_fourier = new_analysis.fourier(acf)  # Спектр Фурье acf производной строки
...
def count_cross_correlation_and_print(diff, xn_cut):
    ...
    for i in range(rows):
        for j in range(cols):
            cross_correlation = new_analysis.cross_correlation(diff[count], diff[count + 1])
            cross_correlation_fourier = new_analysis.fourier(cross_correlation)   # Спектр Фурье ccf производных строк

            ax2[i, j].plot(cross_correlation_fourier)
            ax2[i, j].set_xlim([0, 1 / (2 * dt)])
            ax2[i, j].set_title('row ' + str(count * step) + ' and row ' + str((count + 1) * step), fontsize=20)

            max_point = max(cross_correlation_fourier[0:len(cross_correlation_fourier) // 2])
            index = cross_correlation_fourier[0:len(cross_correlation_fourier) // 2].index(max_point)
            freq = xn_cut[index]
            ...
            ax2[i, j].vlines([freq - indent, freq + indent], 0, max(cross_correlation_fourier), color='gray', linestyle='dashed')
